...
async function labels(info) {
  ...
  previous
    .filter((d) => !expected.find((expect) => d.slug === expect.slug))
    .forEach((d) => {
      if (own.call(replaceMap, d.slug)) {
        const name = replaceMap[d.slug]
        const replace = expected.find((expect) => expect.slug === name)
        const renameable =
          !previous.find((expect) => expect.slug === name) &&
          !change.find((expect) => expect.to.slug === name)
  ...
  const add = expected
    ...
    .filter((d) => !previous.find((expect) => d.slug === expect.slug))
    // Not marked for a change.
    .filter((d) => !change.find((expect) => d.slug === expect.to.slug))
  ...
  previous
    // Not added above.
    .filter((d) => !add.find((expect) => d.slug === expect.slug))
    .forEach((ac) => {
